...
export class CheckoutComponent implements OnInit {
  ...
  updateProductQuantitiesAndPlaceOrder() {
    ...
    forkJoin(updateRequests).subscribe(
      responses => {
        console.log('Tất cả số lượng Món Ăn đã được cập nhật thành công!');
  ...
  placeOrder(){
    ...
    // order
    const {firstname,lastname,country,address,town,state,postCode,phone,email,note,status,bank} = this.orderForm;  
    this.orderService.placeOrder(firstname,lastname,country,address,town,state,this.totalFinal,postCode,phone,email,note,status,bank,this.voucher.money,this.listOrderDetail,this.username).subscribe({
      next: res => {
        console.log(res)
      if (bank === 0) {
