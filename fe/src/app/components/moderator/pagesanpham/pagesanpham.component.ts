...
@Component({
  ...
  styleUrls: ['./pagesanpham.component.css'],
  providers: [MessageService,ConfirmationService]
})
export class PagesanphamComponent implements OnInit {
  ...
  getListProduct(){
    this.productService.getListProductByUser(this.id).subscribe({
      next: res =>{
  ...
  createProduct(){
    ...
    if (data.length === 0) {
      this.showError("Vui lòng thêm ảnh trước khi tạo món ăn.");
      return; // Dừng và không tiếp tục thực hiện các bước tiếp theo
    }
